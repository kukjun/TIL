...
* Serial 통신
  ...
  * `Serial.begin(9600)`
    : 아두이노 보드의 기본 Serial 연결을 사용하겠다고 아두이노에게 알려주는 역할을 한다. 이때 반드시 통신속도를 지정 해주어야 한다. **Baudrate**라고 부르는데 보통 9600이 기본값처럼 쓰인다.
...
주위 밝기가 밝아지면 광자양이 많아져 내부 저항이 감소한다. 
